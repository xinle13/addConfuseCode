...
# 需要修改的类名前缀
pre_str = setFile.pre_str
# 新的类名前缀
pre_to_str = setFile.pre_to_str
# 搜寻以下文件类型
suf_set = setFile.suf_set
# 项目路径
#../
project_path = setFile.project_path
# 项目配置文件路径
pbxpro_path = setFile.pbxpro_path
...
for (root, dirs, files) in os.walk(project_path):
    for dir_name in dirs:
        if dir_name in needModifyDic.keys():
            old_name = os.path.splitext(dir_name)[0]
            new_name = file_rename(os.path.join(root, dir_name))
